...
/*export async function completeTransferExample(amount: string, toAddress: string, contractAddress: string): Promise<any> {
    ...
    const provider = await initProvider('wss://rpc.reefscan.com/ws');
...
export async function completeTransferExample1(amount: string, toAddress: string, contractAddress: string): Promise<any> {
    ...
    const signingKey = new MnemonicSigner('control employ home citizen film salmon divorce cousin illegal episode certain olympic');
    ...
    // const fromAddr = accs[0].address;
    const fromAddr='5EnY9eFwEDcEJ62dJWrTXhTucJ4pzGym4WZ2xcDKiT3eJecP'
    const signer = new Signer(provider, fromAddr, signingKey);
